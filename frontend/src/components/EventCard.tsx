  ...
  return (
    ...
    >
      ...
      <div className="relative h-52 w-full overflow-hidden bg-gray-100"> 
        <img 
  src={event.imageUrl || CATEGORY_IMAGES[event.category?.toLowerCase() as keyof typeof CATEGORY_IMAGES]} 
  alt={event.title} 
  loading="lazy" 
  className="w-full h-full object-cover transition-transform duration-300 hover:scale-105"
/>
        <div className="absolute top-3 left-3 flex gap-2">
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getCategoryColor(event.category)} uppercase tracking-wide shadow-sm`}>
            {event.category}
          </span>
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getStatusColor(event.status)} uppercase tracking-wide shadow-sm`}>
            {event.status}
          </span>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="text-xl font-bold text-gray-900 mb-1 line-clamp-1">{event.title}</h3>
        ...
        <div className="flex items-center text-sm font-medium text-indigo-600 mb-3">
            ...
            <span className="line-clamp-1">{event.collegeName || 'Campus Event'}</span>
        </div>
        ...
        <div className="space-y-2.5 mt-auto mb-4 border-t border-gray-50 pt-4">
          ...
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="font-medium text-gray-700">{event.participantsCount} / {event.maxParticipants}</span>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
            {event.tags?.slice(0, 3).map(tag => (
                <span key={tag} className="inline-flex items-center px-2 py-1 rounded text-[10px] bg-gray-50 text-gray-600 border border-gray-200">
        ...
        <div className="pt-4 border-t border-gray-100 flex justify-between items-center text-[10px] text-gray-400">
            ...
            <span className="text-indigo-600 font-bold hover:underline uppercase tracking-tighter">View Details</span>
        </div>
      </div>
    </div>
  );
