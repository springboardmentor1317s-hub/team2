...
const EventFeedbackAdmin: React.FC<Props> = ({ eventId }) => {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      try {
        const res = await axios.get(`/api/feedback/event/${eventId}`);
  ...
  return (
    <div className="mt-6 border rounded-xl p-4 bg-white">
      ...
      <div className="mt-3 p-2 bg-blue-50 rounded text-xs text-blue-700">
        <strong>📊 Data Storage:</strong> All feedback is permanently stored in MongoDB with timestamps, 
        user references, and event associations for analytics and reporting.
      </div>
    </div>
  );
};
