...
import {
  Users, Calendar, TrendingUp, Activity, AlertCircle, Plus, FileText, CheckCircle, Filter, Shield, MoreHorizontal, Clock, Check, X as XIcon, Download, Home, ChevronRight, LogOut, Settings, ClipboardList
} from 'lucide-react';
import {
...
export function Dashboard({
    ...
    return (
        <div className="dashboard-layout flex h-screen bg-gray-50">
            ...
            <aside className="sidebar w-64 bg-white shadow-xl flex flex-col justify-between">
                <div className="p-4">
                    <div className="logo-section mb-6">
                        <h1 className="text-xl font-extrabold text-indigo-700">CampusEventHub</h1>
                        <p className="text-xs text-gray-500 mt-1 capitalize">{user.role} Portal</p>
                    </div>

                    <nav className="nav-menu space-y-1">
                        {currentTabs.map((tab) => (
                            <button
                                key={tab}
                                onClick={() => setActiveTab(tab.toLowerCase())}
                                className={`w-full flex items-center p-3 rounded-lg font-medium text-sm transition-colors ${
                                    activeTab === tab.toLowerCase()
                                        ? "bg-indigo-500 text-white shadow-md"
                                        : "text-gray-600 hover:bg-gray-100"
                                }`}
                            >
                                {tab === "Overview" ? <Home className="w-5 h-5 mr-3" /> :
                                 tab === "My Events" || tab === "Event Management" ? <Calendar className="w-5 h-5 mr-3" /> :
                                 tab === "User Management" ? <Users className="w-5 h-5 mr-3" /> :
                                 tab === "Registrations" ? <ClipboardList className="w-5 h-5 mr-3" /> :
                                 tab === "Analytics" ? <TrendingUp className="w-5 h-5 mr-3" /> :
                                 tab === "Admin Logs" ? <FileText className="w-5 h-5 mr-3" /> :
                        ...
                        <button
                            ...
                            className="w-full flex items-center p-3 rounded-lg font-medium text-sm transition-colors text-gray-600 hover:bg-gray-100 mt-4 border-t border-gray-100 pt-3"
                        >
                ...
                <div className="logout-section p-4 border-t border-gray-100">
                    <button onClick={handleLogout} className="w-full flex items-center p-3 rounded-lg font-medium text-sm text-red-600 hover:bg-red-50 transition-colors">
